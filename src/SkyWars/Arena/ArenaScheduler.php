        ...
        switch ($this->plugin->phase) {
            case Arena::PHASE_LOBBY:
                if(count($this->plugin->players) >= 2) {
                    $this->plugin->broadcastMessage("§a> Empezando en " . Time::calculateTime($this->startTime) . " sec.", Arena::MSG_TIP);
                    $this->startTime--;
                    if($this->startTime == 0) {
                    ...
                    $this->plugin->broadcastMessage("§c> Necesitas mas jugadores para empezar!", Arena::MSG_TIP);
                    $this->startTime = 40;
                }
                break;
            case Arena::PHASE_GAME:
                $this->plugin->broadcastMessage("§a> Hay " . count($this->plugin->players) . " Jugadores, Hora de terminar: " . Time::calculateTime($this->gameTime) . "", Arena::MSG_TIP);
                switch ($this->gameTime) {
                    case 15 * 60:
                        $this->plugin->broadcastMessage("§a> Todos los cofres se rellenarán en 5 min..");
                        break;
                    case 11 * 60:
                        $this->plugin->broadcastMessage("§a> Todos los cofres se rellenarán en 2 min..");
                        break;
                    case 10 * 60:
                        $this->plugin->broadcastMessage("§a> Todos los cofres han sido rellenados.");
                        break;
                }
            ...
            case Arena::PHASE_RESTART:
                $this->plugin->broadcastMessage("§a> Reiniciando en {$this->restartTime} sec.", Arena::MSG_TIP);
                $this->restartTime--;
